...
| 文件名 | 说明 |
| :--- | :--- |
| `led.v` | **核心设计文件**。包含 LED 点亮的时序判断逻辑，通过调节 `counter` 计数阈值来控制点亮或关闭小灯所需的时钟周期（clk）数。 |
| `tb_led.v` | **测试激励文件 (Testbench)**。用于生成时钟信号（clk）、设置仿真总时长，并配置后续生成的 VCD 波形文件。 |
